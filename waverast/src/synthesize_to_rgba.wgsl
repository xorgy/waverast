//! Inverse Haar wavelet transform — writes sRGB RGBA8 to a storage texture.
//!
//! Each thread computes one pixel's coverage via the wavelet expansion,
//! applies fill rule, sRGB encoding, and white-background inversion.
...
fn synthesize_rgba(@builtin(global_invocation_id) gid: vec3<u32>) {
    let col = gid.x;
    let row = gid.y;
    if col >= params.width || row >= params.height {
        return;
    }
    // Map display pixel to coefficient space via view transform.
    // Identity (scale=1, offset=0) gives the standard px = col / wh.
